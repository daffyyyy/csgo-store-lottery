...
@section('title_body')
    ...
    <span class="text-gray-200 text-lg">Edytowanie <strong>{{ $awards->name }}</strong></span>
@endsection
    ...
    <div class="col-span-9 p-4 md:pt-0 shadow-xl rounded-md dark:bg-gray-800 border border-gray-700">
        <div class="flex items-center justify-between p-4 align-middle">
            <h4 class="text-lg font-semibold text-gray-500 dark:text-light">
                ...
                Edytowanie <strong>{{ $awards->name }}</strong>
            </h4>
        ...
        <div class="relative p-4 text-center">
            <form action="{{ route('admin.awards.update', ['awards' => $awards]) }}" method="POST"
                ...
                <div class="mb-1">
                    ...
                    <input
                        ...
                        id="name" name="name" type="text" value="{{ $awards->name }}">
                </div>
                <div class="mb-1">
                    ...
                    <textarea
                        class="bg-gray-900 shadow appearance-none border border-indigo-600 rounded w-2/3 py-2 px-3 text-gray-300 mb-3 leading-tight focus:outline-none focus:shadow-outline"
                        id="description" name="description" type="text">{{ $awards->description }}</textarea>
                </div>
                <div class="mb-5">
                    ...
                    @if (!empty($awards->image))
                        <img class="inline-block" src="{{ $awards->image }}" />
                    @endif
                </div>
                <div class="mb-1">
                    <input
                ...
                <div class="mb-1">
                    ...
                    <input
                        class="bg-gray-900 shadow appearance-none border border-indigo-600 rounded w-2/3 py-2 px-3 text-gray-300 mb-3 leading-tight focus:outline-none focus:shadow-outline"
                        id="cost" name="cost" type="number" value="{{ $awards->cost }}">
                </div>
                <div class="mb-1">
                    ...
                    <input
                        class="bg-gray-900 shadow appearance-none border border-indigo-600 rounded w-2/3 py-2 px-3 text-gray-300 mb-3 leading-tight focus:outline-none focus:shadow-outline"
                        id="stock" name="stock" type="number" value="{{ $awards->stock }}">
                </div>
                @if ($awards->type === 2)
                    <div class="mb-1" id="days">
                        ...
                        <input
                            class="bg-gray-900 shadow appearance-none border border-indigo-600 rounded w-2/3 py-2 px-3 text-gray-300 mb-3 leading-tight focus:outline-none focus:shadow-outline"
                            id="days" name="days" type="number" value="{{ $awards->days }}">
                    </div>
                @endif
                <div class="mb-1">
                    ...
                    <select name="type" id="type"
                        ...
                        @foreach ($types as $key => $type)
                            <option value="{{ $key }}" {{ $key === $awards->type ? 'selected' : '' }}>
                                {{ $type }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-1 {{$awards->type !== 1 ? 'hidden' : '' }}" id="code">
                ...
                <div class="mb-1">
                    ...
                    <button class="text-white font-bold p-2 lg:px-4 md:mx-2 rounded bg-yellow-600"><i
                            class="fas fa-save"></i> Zapisz</button>
                </div>
            </form>
